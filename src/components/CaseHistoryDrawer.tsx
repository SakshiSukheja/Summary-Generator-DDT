            ...
            filteredHistory.map((item) => (
              ...
              >
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="flex items-center gap-2 text-[11px] text-slate-500 mt-0.5">
                      ...
                      <span className="flex items-center gap-1">
                        <Calendar className="w-3 h-3 text-amber-600" /> {formatDateToDDMMYYYY(item.shootDate)}
                      </span>
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-1.5 pt-1 text-[10px]">
                  {item.summary?.jiraLink && (
                    <a
                      href={item.summary.jiraLink}
                      target="_blank"
                      rel="noopener noreferrer"
                      onClick={(e) => e.stopPropagation()}
                      className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-blue-100 text-blue-800 font-medium hover:bg-blue-200 transition-colors"
                    >
                      <ExternalLink className="w-2.5 h-2.5" /> Jira ↗
                    </a>
                  )}
                  {item.summary?.driveLink && (
                    <a
                      href={item.summary.driveLink}
                      target="_blank"
                      rel="noopener noreferrer"
                      onClick={(e) => e.stopPropagation()}
                      className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-emerald-100 text-emerald-800 font-medium hover:bg-emerald-200 transition-colors"
                    >
                      <FolderSymlink className="w-2.5 h-2.5" /> Drive ↗
                    </a>
                  )}
                </div>
