    ...
    <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            {dateMode === 'single' && (
              <input
                ...
                onChange={(e) => {
                  const val = e.target.value;
                  setStartDate(val);
                  onChange({ shootDate: formatDateToDDMMYYYY(val) });
                }}
                className="w-full bg-slate-50 border border-slate-200 focus:bg-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 rounded-lg px-3 py-2 text-sm text-slate-900 transition-all outline-none"
              />
            )}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      onChange({ shootDate: `${formatDateToDDMMYYYY(val)} to ${formatDateToDDMMYYYY(end)}` });
                    }}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const val = e.target.value;
                      setEndDate(val);
                      onChange({ shootDate: `${formatDateToDDMMYYYY(startDate)} to ${formatDateToDDMMYYYY(val)}` });
                    }}
                    className="w-full bg-slate-50 border border-slate-200 focus:bg-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 rounded-lg px-2.5 py-1.5 text-xs text-slate-900 transition-all outline-none"
                  />
                </div>
            ...
            {dateMode === 'custom' && (
              <input
                ...
                placeholder="e.g. 26-07-2026 to 28-07-2026 or Next Week"
                className="w-full bg-slate-50 border border-slate-200 focus:bg-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 rounded-lg px-3 py-2 text-sm text-slate-900 transition-all outline-none"
              />
            )}
            <p className="text-[10px] text-slate-400 mt-1 font-mono">
              Formatted: <span className="font-semibold text-slate-700">{formatDateToDDMMYYYY(form.shootDate)}</span> (DD-MM-YYYY)
            </p>
          </div>
          ...
          {/* Jira Card Link Input */}
          <div>
            <label className="text-xs font-bold text-slate-700 mb-1 flex items-center gap-1">
              <Link2 className="w-3.5 h-3.5 text-blue-600" /> Jira Card Link
            </label>
            <input
              type="url"
              value={form.jiraLink || ''}
              onChange={(e) => onChange({ jiraLink: e.target.value })}
              placeholder="e.g. https://impactguruinc.atlassian.net/browse/AM-13070"
              className="w-full bg-slate-50 border border-slate-200 focus:bg-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 rounded-lg px-3 py-2 text-xs text-slate-900 placeholder-slate-400 transition-all outline-none"
            />
          </div>
          ...
          <div>
            <label className="text-xs font-bold text-slate-700 mb-1 flex items-center gap-1">
              <FolderSymlink className="w-3.5 h-3.5 text-emerald-600" /> Google Drive Link (for DDT)
            </label>
            <input
              type="url"
