...
export async function processDdtSummaryRequest(body: DDTGenerateRequestBody, apiKey?: string) {
  ...
  const formattedShootDate = formatDateToDDMMYYYY(shootDate) || formatDateToDDMMYYYY(new Date().toISOString().split('T')[0]);
  const finalJiraLink = jiraLink.trim() || 'https://impactguruinc.atlassian.net/browse/AM-13070';
  const finalDriveLink = driveLink.trim() || 'https://drive.google.com/drive/folders/1CXYwsH4VSOmZOR947S5H3hDhD_00TE0v?usp=drive_link';
...
3. Case Metadata (Patient Name: "${patientName}", BD Name: "${bdName}", Shoot Date: "${formattedShootDate}", Shoot Location: "${shootLocation}", Source: "${source}", Notes: "${additionalNotes}").
...
Shoot Date:- ${formattedShootDate} Confirmed with the campaigner
...
Jira Link : ${finalJiraLink}
Google Drive Link: ${finalDriveLink}
